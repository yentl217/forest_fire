...
void fast_search_loop(vector<coord>& new_starts, coord start,int **trees, int length, int width)
{
	coord current_searches[4];
	new_searches(current_searches,start,length,width);
...
void fast_count(int **forest, int length, int width, coord init)
{
	vector<coord> new_starts;
	int cluster_size = 1;
	forest[init.get_x()][init.get_y()] = 1;
	
	fast_search_loop(new_starts,init,forest,length,width);
	while(new_starts.size() !=0 )
	{
		fast_search_loop(new_starts,new_starts[0],forest,length,width);
		forest[new_starts[0].get_x()][new_starts[0].get_y()] = 1;
		cluster_size++;
		new_starts.erase(new_starts.begin());
	}
