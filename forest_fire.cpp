...
//Initialising
int num_steps = 2000;
double prob_f = 1.0-1.0/(16000*16000); //probability of tree NOT burning independent of neighbours
	...
	for (int i = 0; i <num_steps; i++)
	{
		...
		for (int y = 0; y<width; y++)
		{
			...
			{	
				...
				if (forest[y][x] == 1 || forest[y][x] == 4) list_fires.push_back(coord(x,y));
			}
		}
		...
		for (int y=0; y<width; y++)
		{
			for (int x=0; x<length; x++)
			{
				...
				{
					//update all burning trees to empty cells
					if (coord(x,y) == list_fires[i]) forest[y][x] = 0;
				}
				for (int i =0; i<list_trees.size(); i++)
				{
					//update all trees that should burn to burning trees
					if (coord(x,y) == list_trees[i]) forest[y][x] = 1;
				}
			}
		}
		cout << i << endl;
